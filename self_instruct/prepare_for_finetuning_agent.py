...
def encode_instance(instruction, input, output, random_template=True, source="machine", type="agent_function_call"):
...
def parse_instances_for_generation_task(raw_text, instruction, response_metadata, type_info="agent_function_call"):
    instances = []
    raw_text = raw_text.strip()

    # 新规则：必须同时包含 "name" 和 "arguments" 两个 key
    import re
    has_name = re.search(r'\bname\b', raw_text)
    has_args = re.search(r'\barguments\b', raw_text)
